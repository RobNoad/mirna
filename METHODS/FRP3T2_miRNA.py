...
# Regexes          
# Box B sequences: GTTCNRNNC, GGTTSGNG, GKWCAAGTC, GTTCS
box_b_re = re.compile('GTTC[ATCG][AG][ATCG]{2}C|GGTT[GC]G[ATCG]G|GGTT[CG][ATCG]G|GTTC[GC]')
# Box A sequences: TRGNNNNNNGR, TRGNNNNNGR
box_a_re = re.compile('T[AG]G[ATCG]{6,7}G[AG]')
...
print "\t".join(['Accession','Description','Direction','A:B:T','Length','A Box','B Box','Terminator','Sequence','Secondary Structure','FreeEnergy','Stem Loops and Free Energies'])
	...
	for sn,s in ss.iteritems():
		...
		for mb in box_b_re.finditer(s):
			...
			for ma in box_a_re.finditer(s[window_start:mb_pos]):
				...
				for mt in box_t_re.finditer(s[mb_pos-30:mb_pos+window_b_to_t]):
					mt_pos =  mt.start()+mb_pos-30
